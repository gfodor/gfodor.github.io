import Peer from './perfect-negotiation.js'
...
(async function() {
  ...
  dbreq.addEventListener("upgradeneeded", ({ target: { result: db } }) => {
    db.createObjectStore("sdpdata", { keyPath: "id" });
  });

  dbreq.addEventListener("success", ({ target: { result  } }) => {
    db = result;

    db.transaction("sdpdata").objectStore("sdpdata").get(CERT_DB_KEY).addEventListener("success", async ({ target: { result } }) => {
      // TODO check expiration
      if (result) {
        const peer = new Peer({ certificates: [result.cert], trickle: false, polite: true });
        ...
        console.log(cert);
        db.transaction("sdpdata", "readwrite").objectStore("sdpdata").put({ id: CERT_DB_KEY, cert });
      }
    })
  });
})();
...
//      await import('https://jimmy.warting.se/packages/webpush/encryption-aes128gcm.js')
//
...
//  const { AnalogClock } = await import('../../packages/dummycontent/canvas-clock.js')
